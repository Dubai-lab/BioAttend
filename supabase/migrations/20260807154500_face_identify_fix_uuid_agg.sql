-- ============================================================================
-- BioAttend · Migration 08 — identify_face: pick the top row without max(uuid)
--
-- Migration 07 used `max(staff_id) filter (where position = 1)`, which fails:
--     function max(uuid) does not exist
--
-- Postgres has no max() aggregate for uuid. The intent was only ever "take the
-- top-ranked row", so select it directly and pull the runner-up as a
-- correlated subquery over the same CTE.
--
-- Verified alongside: debug_face_scores returns 1.0 when a stored descriptor
-- is fed back as the query, so the vector comparison itself is correct.
--
-- Safe to re-run.
-- ============================================================================
